...
describe('userGetService', () => {
  ...
  it('get user1 by guest', async () => {
    ...
    const user = await userGetService(uc, GUEST, 1, err)
  ...
  it('get user1 by user1', async () => {
    const err: ErrorConst[] = []
    const user = await userGetService(uc, USER1, 1, err)
    if (!user) throw new Error()
    expect(user as any).toEqual({
      id: 1,
      name: 'User 1',
  ...
  it('get user1 by admin', async () => {
    const err: ErrorConst[] = []
    const user = await userGetService(uc, ADMIN, 1, err)
    if (!user) throw new Error()
    expect(user as any).toEqual({
      id: 1,
      name: 'User 1',
  ...
  it('get invalid', async () => {
    const err: ErrorConst[] = []
    const user = await userGetService(uc, USER1, 99, err)
