...
import { checkHomeUsable, HOME_DUPE } from './user-form.js'
...
describe('UserForm', () => {
  ...
  describe('checkHomeUsable', () => {
    it('should ok when one entity', async () => {
      ...
      await checkHomeUsable(udb, 1, 'User 1', errs)
      expect(errs.length).toBe(0)
    })
    it('should ok when one entity 2', async () => {
      const errs: FormError[] = []
      await checkHomeUsable(udb, 1, 'user1', errs)
      expect(errs.length).toBe(0)
    })
    it('should ok when valid', async () => {
      const errs: FormError[] = []
      await checkHomeUsable(udb, 0, 'alice', errs)
      expect(errs.length).toBe(0)
    })
    it('should fail when is in use', async () => {
      const errs: FormError[] = []
      await checkHomeUsable(udb, 0, 'user1', errs)
      expect(errs).toContain(HOME_DUPE)
    })
    it('should fail when is in use 2', async () => {
      const errs: FormError[] = []
      await checkHomeUsable(udb, 0, 'User 1', errs)
      expect(errs).toContain(HOME_DUPE)
    })
  })
