...
import { PwResetDB, PwResetRecord } from '../../db/pwreset-db.js'
import { FormError, INVALID_DATA } from '../../../lib/base/error2.js'
import { checkPasswordFormat, EMAIL_NOT_FOUND, EMAIL_PATTERN } from '../register-form/user-form.js'
...
export async function pwResetSendMailService(
  ...
  const mail = {
    ...
    text:
      ...
      config.siteUrl + '/user/password-reset-3?uuid=' + r.uuid + '&t=' + r.token + '\n\n' +
      config.appName
  }
