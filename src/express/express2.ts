...
import type { ErrorRequestHandler, Express, NextFunction, Request, Response, Router } from 'express'
import bodyParser from 'body-parser'
import cookieParser from 'cookie-parser'
import redis from 'redis'
import expressSession from 'express-session'
import * as http from 'http'
import { newErrorConst } from '../common/util/error2.ts'
import type { Config } from '../common/type/config.ts'
import { inDev, inProduction } from '../common/util/env2.ts'
...
import { renderJson } from './response.ts'
...
export class Express2 {
  ...
  private setUpErrorHandler() {
    this.express.use((err: any, req: Request, res: Response, done: NextFunction) => {
      if (this.logError) {
        console.error(err)
      }
      this.errorHandler(err, req, res, done)
    })
  }
...
function defaultErrorHandler(err: any, req: Request, res:Response, done:NextFunction) {
